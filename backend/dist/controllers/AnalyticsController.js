...
const StatisticalEngine_1 = require("../services/StatisticalEngine");
class AnalyticsController {
    // Endpoint protegido: Obter o dashboard de uma pesquisa específica
    getSurveyAnalytics(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const survey = yield prismaClient_1.prisma.survey.findFirst({
                    where: { id, tenantId },
                    include: {
                        questions: true,
                        submissions: {
                ...
                const riskBySector = Object.keys(sectorStats).map(sector => {
                    ...
                    if (avgRisk >= 6)
                        riskLevel = 'Alto (Ação imediata)';
                    else if (avgRisk >= 3)
                        riskLevel = 'Médio (Melhorar controles)';
                    return {
                        sector,
    ...
    generateAIReport(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const cronbachAlpha = StatisticalEngine_1.StatisticalEngine.calculateCronbachAlpha(submissionsMatrix);
                ...
                const globalStats = StatisticalEngine_1.StatisticalEngine.analyzeRiskDistribution(allDataPoints);
                const sectorsComputed = Object.entries(sectorDataPoints).map(([sector, dataPoints]) => {
                    const stats = StatisticalEngine_1.StatisticalEngine.analyzeRiskDistribution(dataPoints);
                    return { sector, stats };
                ...
                if (criticalSectors.length > 0) {
                    ...
                    criticalSectors.forEach(s => {
                        ...
                        if (s.stats.isPolarized)
                            report += ` | ⚠️ Polarizado: Desvio Padrão \`${s.stats.standardDeviation.toFixed(2)}\``;
                        report += `)\n`;
                    });
